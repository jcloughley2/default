<div class="clear"></div>
</div>
<footer id="footer" role="contentinfo">
	<div class="footer-mountain">
		<div class="footer-mountain-inner">
			<svg class="mountain-range" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 480 78" style="enable-background:new 0 0 480 78;" xml:space="preserve">
			<polygon points="0,78 55.5,55 80,66.5 103,46 112.5,46 140,22 179,57.5 201.5,50 208,34 236,37.5 281,0 300.5,17 319,7 335.5,30 
			346,22.5 384.5,58.5 396,51 426.5,68.5 480,78 "/>
			</svg>
		</div>
	</div>

	<div class="footer-text">
		<div class="footer-text-inner">
			<div class="info">
				<h2>FiveTwelve, LLC</h2>
				<span>info@fivetwelve.com</span>
				<span>FiveTwelve, LLC</span>
				<a href="#">Link Placeholder</a>
				<a href="#">Link Placeholder</a>
				<a href="#">Contact Us</a>
			</div>
			<div class="social">
				<a class="social-link facebook" href="#"><span>Facebook</span></a>
				<a class="social-link linkedin" href="#"><span>Linkedin</span></a>
			</div>		
		</div>
	</div>
</footer>
</div>
<?php wp_footer(); ?>
</body>
</html>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
<script src="<?php echo get_stylesheet_directory_uri(); ?>/js/all.js"></script>  